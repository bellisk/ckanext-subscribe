...
from ckanext.subscribe.helpers import string_to_timedelta
...
def send_weekly_notifications_if_its_time_to():
    ...
    if not notifications_by_email:
        ...
        log.debug(
            "sending {} emails (weekly frequency)".format(len(notifications_by_email))
        )
        send_emails(notifications_by_email)
...
def send_daily_notifications_if_its_time_to():
    ...
    if not notifications_by_email:
        ...
        log.debug(
            "sending {} emails (daily frequency)".format(len(notifications_by_email))
        )
        send_emails(notifications_by_email)
