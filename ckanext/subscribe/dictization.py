...
def subscription_save(subscription_dict, context):
    subscription_obj = table_dict_save(subscription_dict, Subscription, context)
...
def dictize_subscription(subscription_obj, context, include_name=False):
    ...
    if include_name:
        if subscription_dict["object_type"] == "dataset":
            subscription_dict["object_name"] = model.Package.get(
                subscription_dict["object_id"]
            ).id
        else:
            subscription_dict["object_name"] = model.Group.get(
                subscription_dict["object_id"]
            ).id

    subscription_dict["frequency"] = Frequency(subscription_dict["frequency"]).name
